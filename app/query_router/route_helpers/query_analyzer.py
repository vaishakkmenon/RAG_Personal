...
class QueryAnalyzer:
    ...
    def _check_ambiguity(self, analysis: Dict[str, Any]) -> None:
        ...
        SIMPLIFIED: All ambiguity detection disabled. Queries are marked as NOT ambiguous
        and the generation LLM handles ambiguity naturally through the system prompt.

        This eliminates the expensive LLM ambiguity detection call and fast-path heuristics,
        reducing cost and latency by ~50% while maintaining quality through prompt engineering.
        """
        logger.debug("Ambiguity detection disabled - letting generation LLM handle naturally")
        analysis.update({
            "ambiguity_method": "disabled",
            "is_ambiguous": False,
            "confidence": 1.0,
            "needs_clarification": False
        })
