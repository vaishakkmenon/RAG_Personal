...
class QualityReportGenerator:
    ...
    def _generate_summary(
        ...
        passed_with_issues = sum(
            1 for r in analysis_results if r["test_result"]["passed"] and r["issues"]
        )
        ...
        return {
            ...
            "false_positive_rate": (passed_with_issues / passed_tests * 100)
            if passed_tests > 0
            else 0,
    ...
    def _analyze_by_category(
            ...
            if result["issues"]:
                ...
                for issue in result["issues"]:
                    ...
                    by_category[category]["common_issue_types"][issue_type] = (
                        by_category[category]["common_issue_types"].get(issue_type, 0)
                        + 1
                    )
        ...
        for category, stats in by_category.items():
            stats["issue_rate"] = (
                (stats["tests_with_issues"] / stats["total_tests"] * 100)
    ...
    def _severity_distribution(
        ...
        return {
            "CRITICAL": {
                "count": len(distribution["CRITICAL"]),
                "issues": distribution["CRITICAL"],
            },
            "HIGH": {
                "count": len(distribution["HIGH"]),
                "issues": distribution["HIGH"],
            },
            "MEDIUM": {
                "count": len(distribution["MEDIUM"]),
                "issues": distribution["MEDIUM"],
            },
    ...
    def to_console(self, report: Dict[str, Any]) -> str:
        ...
        lines.append(
            f"Tests with quality issues: {summary['tests_with_quality_issues']}"
        )
        lines.append(f"Total quality issues found: {summary['total_quality_issues']}")
        lines.append(
            f"False positives (passed with issues): {summary['false_positives']}"
        )
        ...
        for issue_type, count in sorted(
            summary["issue_type_counts"].items(), key=lambda x: x[1], reverse=True
        ):
    ...
    def to_markdown(self, report: Dict[str, Any]) -> str:
        ...
        lines.append(
            f"- **False positives:** {summary['false_positives']} ({summary['false_positive_rate']:.1f}%)"
        )
        ...
        if critical:
            ...
            for finding in critical:
                ...
                lines.append(
                    f"- **Passed:** {finding['passed']}, **Score:** {finding['score']:.2f}\n"
                )
        ...
        for category, stats in sorted(category_analysis.items()):
            lines.append(
                f"| {category} | {stats['total_tests']} | {stats['tests_with_issues']} | {stats['issue_rate']:.1f}% |"
            )
