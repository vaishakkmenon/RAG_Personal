...
def check_grounding_compliance(
    test_result: Dict[str, Any], test_definition: Dict[str, Any]
) -> List[Dict[str, Any]]:
    """
        ...
        issues.append(
            {
                ...
                "actual": grounded,
                "impact": "Factual answer without source grounding"
                if grounded_expected
                else "Answer grounded when it should not be",
            }
        )
...
def check_impossible_behavior(
    ...
    is_truly_impossible = any(
        ind in question_lower for ind in truly_impossible_indicators
    )
...
def check_ambiguity_behavior(
    ...
    should_ask_clarification = expected_behavior.get("expected_response", {}).get(
